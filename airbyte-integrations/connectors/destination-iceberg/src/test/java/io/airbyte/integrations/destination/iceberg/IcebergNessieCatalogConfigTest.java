...
class IcebergNessieCatalogConfigTest {
  ...
  private static final String AUTH_TYPE = "NONE";
  ...
  void setup() {
    ...
    JsonNode jsonNode = Jsons.jsonNode(ofEntries(entry(IcebergConstants.NESSIE_CATALOG_URI_CONFIG_KEY, FAKE_NESSIE_URI),
        ...
        entry(IcebergConstants.NESSIE_CATALOG_AUTHENTICATION_TYPE_KEY, AUTH_TYPE)));
  ...
  @Test
  public void checksNessieAuthType() {
    final IcebergDestination destinationFail = new IcebergDestination();
    final AirbyteConnectionStatus status = destinationFail.check(Jsons.deserialize("""
                                                                                   {
                                                                                     "catalog_config": {
                                                                                       "catalog_type": "NESSIE",
                                                                                       "nessie_uri": "http://fake-nessie-uri",
                                                                                       "nessie_auth_type": "INVALID",
                                                                                       "database": "test"
                                                                                     },
                                                                                     "storage_config": {
                                                                                       "storage_type": "S3",
    ...
    log.info("status={}", status);
    assertThat(status.getStatus()).isEqualTo(Status.FAILED);
    assertThat(status.getMessage()).contains("[NONE, BEARER] are the only supported authentication types");
  }
  ...
  public void nessieCatalogSparkConfigTest() {
    ...
    assertThat(sparkConfig.get("spark.sql.catalog.iceberg.authentication.type")).isEqualTo(AUTH_TYPE);
